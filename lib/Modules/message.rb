...
module Message
  ...
  def back_exit_or_open_message
    puts "\nType 'open' to #{'open'.white.bold} this listing in browser, " \
         "type 'back' to go #{'back'.cyan} to the listings page, " \
         "or type 'exit' to #{'exit'.red}.\n\n"
  end
  ...
  def display_price_insights_info(property)
    puts "\n#{light_black_bold('List Price:')}       #{property.price} " \
  ...
  def see_price_insights_message
